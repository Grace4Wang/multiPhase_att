...
# x3 = data11['x3'][max_id]
# x4 = data11['x4']
# val_midpre = data11['val_midpre'][max_id]
pred = data11['pred'][max_id]
...
pred1 = np.average(pred, axis=0)  # 按列求均值
#
# # pred = pred.cpu().numpy()
# # cv2.imwrite('3P.png',pred1)
# # plt.imshow(x2)
pred1 = np.expand_dims(pred1, axis=0)  # or axis=1
plt.yticks([])

plt.imshow(pred1)
# 保存中间值
writer = pd.ExcelWriter('3P_5_5_294_concatSingle.xlsx')
midpre = pd.DataFrame(pred)
midpre.to_excel(writer, 'sheet_1', header='pred', index=False)
