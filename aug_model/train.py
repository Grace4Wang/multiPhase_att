...
from utils import create_lr_scheduler, get_params_groups, evaluate
...
def main(args):
    ...
    # 单期像训练
    train_dataset = MyDataSet(data_path_train1, train_txt_path, transform=data_transform["train"])

    # 实例化验证数据集
    # val_dataset = MyDataSet(data_path_val1, data_path_val2, data_path_val3,  val_txt_path)
    # 单期像验证
    val_dataset = MyDataSet(data_path_val1, val_txt_path, transform=data_transform["val"])
    ...
    if os.path.exists("./tempWeights") is False:    #设置权重保存路径
        os.makedirs("./tempWeights")
    ...
    for epoch in range(args.epochs):
        ...
        for step, data in enumerate(train_data_loader):
            ...
            # batch1 = images1
            # # batch = np.expand_dims(batch, 1)  # if numpy array
            # batch1 = batch1.unsqueeze(1)              # if torch tensor
            # output = model_3D(batch1.to(device)[0])
            ...
            np.savez(result_path + '/result_' + name + '.npz', train_acc=train_acc_, train_loss=train_loss_,
                     ...
                     train_midpre=train_midpre)

            if iter_num % 10 == 0:  #每10个迭代验证一次

                #validation
                val_data_loader = tqdm(val_loader)
                val_loss, val_acc, fpr, tpr, AUC, confu_matrix,sensitivity,specificity,val_trues,val_preds,val_midpre = evaluate(model, val_data_loader, device)
                ...
                print("[val iteration {}] loss: {:.3f}, acc: {:.3f}, sensitivity: {:.3f}, specificity: {:.3f} ".format(iter_num,val_loss,val_acc,sensitivity,specificity))
    ...
    result_path = 'runs_ck19' # change the result storage path
    if os.path.exists(result_path) == False:
        os.makedirs(result_path)

    np.savez(result_path + '/result_' + name + '.npz', val_acc=val_acc_, val_loss=val_loss_, val_confu_matrix=confu_matrix_,
                val_AUC=AUC_, val_fpr=fpr_, val_tpr=tpr_,sensitivity=sensitivity_,specificity=specificity_,train_acc=train_acc_,train_loss=train_loss_,val_trues=val_trues_, val_preds=val_preds_,val_midpre=val_midpre_)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_iter', type=int, default=15001)
    ...
    parser.add_argument('--train_txt_path', type=str,
                        default=r'H:\Bayer\bbox_ing\aug_model\txt_ck19\train_' + fold + '.txt') # change the training directory to ck19
    parser.add_argument('--val_txt_path', type=str,
                        default=r'H:\Bayer\bbox_ing\aug_model\txt_ck19\val_' + fold + '.txt') # change the training directory to ck19
