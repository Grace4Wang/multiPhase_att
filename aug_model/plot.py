...
data11 = np.load('runs/result_' + str(time1+1) + '_' + str(fold1+1) + '_'+labels[0]+'.npz', allow_pickle=True)

data12 = np.load('runs/result_' + str(time2+1) + '_' + str(fold2+1) + '_'+labels[0]+'.npz', allow_pickle=True)
...
plt.plot(val_fpr1, val_tpr1, 'r*-', label=labels[0] + ', AUC = ' + str(best_AUC), linewidth=2.2)
