...
import numpy as np
from my_dataset_fusion_2P import MyDataSet
# from my_dataset import MyDataSet
from LeNetAtt_2P import LeNetAtt as create_model
# from LeNet5_3d import LeNet as create_model
from utils_2P import create_lr_scheduler, get_params_groups, evaluate, roc_auc
...
def main(args):
    ...
    # 实例化训练数据集
    # 三时期期像融合
    train_dataset = MyDataSet(data_path_train1, data_path_train3, train_txt_path)
    # 单期像训练
    # train_dataset = MyDataSet(data_path_train4, train_txt_path, transform=data_transform["train"])

    # 实例化验证数据集
    val_dataset = MyDataSet(data_path_val1, data_path_val3, val_txt_path)
    # 单期像验证
    ...
    if os.path.exists("./weights_2P") is False:    #设置权重保存路径
        os.makedirs("./weights_2P")
    ...
    train_x1_,train_x2_,train_pred_ = [],[],[]
    val_x1_,val_x2_,val_pred_ = [],[],[]
    for epoch in range(args.epochs):
        ...
        for step, data in enumerate(train_data_loader):
            ...
            pred, x1, x2, temp = model(images1.to(device), images2.to(device))
            ...
            result_path = 'trainRuns_2Pchanged'
            ...
            np.savez(result_path + '/result_' + name + '.npz', train_acc=train_acc_, train_loss=train_loss_,
                     ...
                     train_midpre=train_midpre,x1=train_x1_,x2=train_x2_,pred=train_pred_)

            if iter_num % 10 == 0:  #每10个迭代验证一次

                #validation
                val_data_loader = tqdm(val_loader)
                val_loss, val_acc, fpr, tpr, AUC, confu_matrix,sensitivity,specificity,val_trues,val_preds,val_midpre,val_x1,val_x2,valpred = evaluate(model, val_data_loader, device)
                ...
                if best_acc <= val_acc:  # 最好的acc时保存模型权重
                    torch.save(model.state_dict(), "./weights_2P/" + name + ".pth")
                    best_acc = val_acc
    ...
    np.savez(result_path + '/result_' + name + '.npz', train_acc=train_acc_, train_loss=train_loss_, train_AUC=train_AUC_,train_confumatrix=train_confumatrix_, val_acc=val_acc_, val_loss=val_loss_, val_confu_matrix=confu_matrix_,
                val_AUC=AUC_, val_fpr=fpr_, val_tpr=tpr_,sensitivity=sensitivity_,specificity=specificity_,val_trues=val_trues_, val_preds=val_preds_,val_midpre=val_midpre_,
                x1=val_x1_,x2=val_x2_,pred=val_pred_)
...
if __name__ == '__main__':
    ...
    # 'H:\Bayer\bbox_ing\aug_model\weights_5\3_3_AP_PPatt.pth'
