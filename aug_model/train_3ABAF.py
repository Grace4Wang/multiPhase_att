...
from LeNetAtt_ABAF import LeNetAtt as create_model
# from LeNet5_3d import LeNet as create_model
from utils_3ABAF import create_lr_scheduler, get_params_groups, evaluate, roc_auc
...
def main(args):
    ...
    if os.path.exists("./weights_AB") is False:    #设置权重保存路径
        os.makedirs("./weights_AB")
    ...
    for epoch in range(args.epochs):
        ...
        for step, data in enumerate(train_data_loader):
            ...
            pred, loss, L1, L2, L3 = model(images1.to(device), images2.to(device), images3.to(device), labels.to(device))
            ...
            train_data_loader.desc = "[train iteration {}] loss: {:.3f}, acc: {:.3f}, lr: {:.5f}, L1: {:.3f}, L2: {:.3f}, L3: {:.3f}".format(
                iter_num,
                train_loss,
                train_acc,
            ...
            if iter_num % 10 == 0:  #每10个迭代验证一次
                ...
                val_loss, val_acc, fpr, tpr, AUC, confu_matrix,sensitivity,specificity,L1,L2,L3,val_trues,val_preds,val_midpre = evaluate(model, val_data_loader, device)
                ...
                print("[val iteration {}] loss: {:.3f}, acc: {:.3f}, AUC: {:.3f}, L1: {:.3f}, L2: {:.3f}, L3: {:.3f}".format(iter_num, val_loss, val_acc, AUC, L1,L2,L3))
    ...
    np.savez(result_path + '/result_' + name + '.npz', train_acc=train_acc_, train_loss=train_loss_, val_acc=val_acc_, val_loss=val_loss_, val_confu_matrix=confu_matrix_,
                val_AUC=AUC_, val_fpr=fpr_, val_tpr=tpr_, sensitivity=sensitivity_, specificity=specificity_, L1_=L1_, L2_=L2_, L3_=L3_,val_trues=val_trues_, val_preds=val_preds_,val_midpre=val_midpre_)
