...
class _NonLocalBlockND(nn.Module):
    def __init__(self, in_channels, inter_channels=None,sub_sample=True, bn_layer=True):
        ...
        if bn_layer:
            self.W13 = nn.Sequential(
                conv_nd1(in_channels=self.inter_channels, out_channels=self.in_channels,
                         kernel_size=1, stride=1, padding=0),
                bn1(self.in_channels)
            )
            nn.init.constant_(self.W13[1].weight, 0)
            nn.init.constant_(self.W13[1].bias, 0)
        else:
            self.W13 = conv_nd1(in_channels=self.inter_channels, out_channels=self.in_channels,
                               kernel_size=1, stride=1, padding=0)
            nn.init.constant_(self.W13.weight, 0)
        ...
        if bn_layer:
            ...
            nn.init.constant_(self.W23[1].weight, 0)
            nn.init.constant_(self.W23[1].bias, 0)
        else:
            self.W23 = conv_nd2(in_channels=self.inter_channels, out_channels=self.in_channels,
                             kernel_size=1, stride=1, padding=0)
            nn.init.constant_(self.W23.weight, 0)
            nn.init.constant_(self.W23.bias, 0)
        ...
        if bn_layer:
            self.W33 = nn.Sequential(
                conv_nd3(in_channels=self.inter_channels, out_channels=self.in_channels,
                         kernel_size=1, stride=1, padding=0),
                bn3(self.in_channels)
            )
            nn.init.constant_(self.W33[1].weight, 0)
            nn.init.constant_(self.W33[1].bias, 0)
        else:
            self.W33 = conv_nd3(in_channels=self.inter_channels, out_channels=self.in_channels,
                               kernel_size=1, stride=1, padding=0)
            nn.init.constant_(self.W33.weight, 0)
            nn.init.constant_(self.W33.bias, 0)
        ...
        if bn_layer:
            self.W43 = nn.Sequential(
                conv_nd4(in_channels=self.inter_channels, out_channels=self.in_channels,
                         kernel_size=1, stride=1, padding=0),
                bn4(self.in_channels)
            )
            nn.init.constant_(self.W43[1].weight, 0)
            nn.init.constant_(self.W43[1].bias, 0)
        else:
            self.W43 = conv_nd2(in_channels=self.inter_channels, out_channels=self.in_channels,
                                kernel_size=1, stride=1, padding=0)
            nn.init.constant_(self.W43.weight, 0)
            nn.init.constant_(self.W43.bias, 0)
    ...
    def forward(self, x1, x2, x3, x4, return_nl_map=False):
        ...
        # modal 14
        f13 = torch.matmul(theta_x1, phi_x4)
        N13= f13.size(-1)
        f_div_C13 = f13 / N13

        y13 = torch.matmul(f_div_C13, g_x4)
        y13 = y13.permute(0, 2, 1).contiguous()
        y13 = y13.view(batch_size1, self.inter_channels, *x1.size()[2:])
        W_y13 = self.W13(y13)
        z13 = W_y13 + z12
        ...
        # modal 24
        f23 = torch.matmul(theta_x3, phi_x4)
        N23= f23.size(-1)
        f_div_C23 = f23 / N23

        y23 = torch.matmul(f_div_C23, g_x4)
        y23 = y23.permute(0, 2, 1).contiguous()
        y23 = y23.view(batch_size2, self.inter_channels, *x2.size()[2:])
        W_y23 = self.W23(y23)
        z23 = W_y23 + z22
        ...
        # modal 32
        f32 = torch.matmul(theta_x3, phi_x2)
        N32 = f32.size(-1)
        f_div_C32 = f32 / N32

        y32 = torch.matmul(f_div_C32, g_x2)
        ...
        # modal 34
        f33 = torch.matmul(theta_x3, phi_x4)
        N33 = f33.size(-1)
        f_div_C33 = f33 / N33

        y33 = torch.matmul(f_div_C33, g_x4)
        y33 = y33.permute(0, 2, 1).contiguous()
        y33 = y33.view(batch_size3, self.inter_channels, *x3.size()[2:])
        W_y33 = self.W33(y33)
        z33 = W_y33 + z32
        ...
        # modal 42
        f42 = torch.matmul(theta_x4, phi_x2)
        N42 = f42.size(-1)
        f_div_C42 = f42 / N42

        y42 = torch.matmul(f_div_C42, g_x2)
        ...
        # modal 43
        f43 = torch.matmul(theta_x4, phi_x3)
        N43 = f43.size(-1)
        f_div_C43 = f43 / N43

        y43 = torch.matmul(f_div_C43, g_x3)
        y43 = y43.permute(0, 2, 1).contiguous()
        y43 = y43.view(batch_size4, self.inter_channels, *x4.size()[2:])
        W_y43 = self.W43(y43)
        z43 = W_y43 + z42
        
        if return_nl_map:
            return z13,f_div_C11,f_div_C12,f_div_C13, z23,f_div_C21,f_div_C22,f_div_C23, z33,f_div_C31,f_div_C32,f_div_C33, z43,f_div_C41, f_div_C42,f_div_C43
        return z13,z23,z33,z43
