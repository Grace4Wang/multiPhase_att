...
import numpy as np
from my_dataset_4fusion import MyDataSet
# from my_dataset import MyDataSet
from LeNetAtt_4f import LeNetAtt as create_model
# from LeNet5_3d import LeNet as create_model
from utils_4F import create_lr_scheduler, get_params_groups, evaluate
...
def main(args):
    ...
    # 实例化训练数据集
    # 四时期期像融合
    train_dataset = MyDataSet(data_path_train1, data_path_train2, data_path_train3, data_path_train4, train_txt_path)
    # 单期像训练
    # train_dataset = MyDataSet(data_path_train4, train_txt_path, transform=data_transform["train"])

    # 实例化验证数据集
    val_dataset = MyDataSet(data_path_val1, data_path_val2, data_path_val3, data_path_val4, val_txt_path)
    # 单期像验证
    ...
    val_midpre_ = []
    train_x1_,train_x2_,train_x3_,train_x4_,train_pred_ = [],[],[],[],[]
    val_x1_,val_x2_,val_x3_,val_x4_,val_pred_ = [],[],[],[],[]
    ...
    for epoch in range(args.epochs):
        ...
        for step, data in enumerate(train_data_loader):
            ...
            np.savez(result_path + '/result_' + name + '.npz', train_acc=train_acc_, train_loss=train_loss_,
                     ...
                     train_midpre=train_midpre,x1=train_x1_,x2=train_x2_,x3=train_x3_,x4=train_x4_,pred=train_pred_)

            if iter_num % 10 == 0:  #每10个迭代验证一次

                #validation
                val_data_loader = tqdm(val_loader)
                val_loss, val_acc, fpr, tpr, AUC, confu_matrix,sensitivity,specificity,val_trues,val_preds,val_midpre,val_x1,val_x2,val_x3,val_x4,valpred = evaluate(model, val_data_loader, device)
                ...
                val_x4_.append(val_x4)
                val_pred_.append(valpred)

                sensitivity_.append(sensitivity)
                specificity_.append(specificity)

                print("[val iteration {}] loss: {:.3f}, acc: {:.3f}".format(iter_num,val_loss,val_acc))
    ...
    np.savez(result_path + '/result_' + name + '.npz', val_acc=val_acc_, val_loss=val_loss_, val_confu_matrix=confu_matrix_,
                val_AUC=AUC_, val_fpr=fpr_, val_tpr=tpr_,sensitivity=sensitivity_,specificity=specificity_,train_acc=train_acc_,train_loss=train_loss_,
                val_trues=val_trues_, val_preds=val_preds_,val_midpre=val_midpre_,x1=val_x1_,x2=val_x2_,x3=val_x3_,x4=val_x4_,pred=val_pred_)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default=r'H:\Bayer\bbox_ing\aug_model\weights\5_5_fusion_4P.pth')
