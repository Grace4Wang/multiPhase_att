...
if __name__ == '__main__':
    # name = args.name
    # 保存路径
    savefile = 'H:/Bayer/data/heatmap/3P'
    # 验证集
    filepath1 = 'H:/Bayer/data/val/AP'
    filepath2 = 'H:/Bayer/data/val/PP'
    filepath3 = 'H:/Bayer/data/val/HBP'
    # 叠加
    ...
    # load image
        ...
        val_txt_path = r'H:\Bayer\bbox_ing\aug_model\txt\label.txt'
        path_net = os.path.join("H:/Bayer/bbox_ing/aug_model/weights/2_5_fusion_3P.pth") # in this example not use
        ...
        # forward
        output = net(input_tensor1.to(device),input_tensor2.to(device),input_tensor3.to(device))
        # data1.extend(output.detach().numpy())
        # np.savetxt("test_3Fu.csv", data1)
        idx = np.argmax(output.cpu().data.numpy())
        predict.extend(classes[idx])
        # data2 = predict.numpy()
        np.savetxt("predict_3P.csv", predict, fmt='%s')
